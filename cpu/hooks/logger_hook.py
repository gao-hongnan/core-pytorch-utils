    ...
    **The hook has the lowest priority (level 10)**.
    ...
    def _write_console(self) -> None:
        ...
        if self.trainer.train_by_epoch:
            process_string = "Epoch: [{}][{}/{}]".format(
                self.trainer.cur_epoch,
                self.trainer.inner_iter,
                self.trainer.epoch_len - 1,
            )
        else:
            process_string = "Iter: [{}/{}]".format(
                self.trainer.cur_iter, self.trainer.max_iters - 1
            )
        ...
        logger.info(
            "{process}{eta}{losses}{iter_time}{data_time}{lr}{memory}".format(
                ...
                iter_time=space + f"iter_time: {iter_time:.4f}"
                if iter_time is not None
                else "",
                data_time=space + f"data_time: {data_time:.4f}  "
                if data_time is not None
                else "",
                lr=space + f"lr: {lr:.5g}" if lr is not None else "",
                memory=space + f"max_mem: {max_mem_mb:.0f}M"
