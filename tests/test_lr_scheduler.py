...
def get_lrs_d2(max_epochs, epoch_len, opt_cfg, sche_cfg):
    ...
    sche = WarmupParamScheduler(
        sche,
        sche_cfg["warmup_factor"],
        min(sche_cfg["warmup_iters"] / max_iters, 1.0),
        "linear",
    )
...
def _get_optimizer_from_config(opt_cfg):
    ...
    for i in range(opt_cfg["num_pram_groups"]):
        params.append(
            {"params": nn.Parameter(torch.zeros(0)), "lr": opt_cfg["base_lr"] * (i + 1)}
        )
...
def get_lrs_timm(max_epochs, epoch_len, opt_cfg, sche_cfg, epoch_metircs=None):
    ...
    if type == "step":
        ...
        lr_scheduler = PlateauLRScheduler(
            optimizer,
            decay_rate=0.1,
            patience_t=sche_cfg["patience_t"],
            mode="min",
            warmup_t=sche_cfg["warmup_t"],
            warmup_lr_init=sche_cfg["warmup_lr_init"],
        )
...
def get_lrs_cpu(max_epochs, epoch_len, opt_cfg, sche_cfg, epoch_metircs=None):
    ...
    if type == "step":
        torch_scheduler = StepLR(
            optimizer, step_size=sche_cfg.pop("step_size"), gamma=0.1
        )
    elif type == "multistep":
        torch_scheduler = MultiStepLR(
            optimizer, milestones=sche_cfg.pop("milestones"), gamma=0.1
        )
    elif type == "cosine":
        ...
        torch_scheduler = CosineAnnealingWarmRestarts(
            optimizer, T_0=sche_cfg.pop("T_0")
        )
    elif type == "plateau":
        torch_scheduler = ReduceLROnPlateau(
            optimizer, patience=sche_cfg.pop("patience")
        )
...
def get_lrs_torch(max_epochs, epoch_len, opt_cfg, sche_cfg, epoch_metircs=None):
    ...
    if type == "step":
        ...
        lr_scheduler = MultiStepLR(
            optimizer, milestones=sche_cfg["milestones"], gamma=0.1
        )
    elif type == "cosine":
...
def test_no_warmup():
    ...
    for num_pram_groups in [1, 2]:
        ...
        lrs_cpu = get_lrs_cpu(
            max_epochs, epoch_len, opt_cfg, sche_cfg.copy(), epoch_metrics
        )
        lrs_torch = get_lrs_torch(
            max_epochs, epoch_len, opt_cfg, sche_cfg, epoch_metrics
        )
        assert allclose(lrs_cpu, lrs_torch)
    ...
    for by_epoch in [True, False]:
        for num_pram_groups in [1, 2]:
            ...
            if by_epoch:
                ...
                lrs_cpu = get_lrs_cpu(
                    epoch, epoch_len, opt_cfg, sche_cfg, epoch_metrics
                )
    ...
    for by_epoch in [True, False]:
        for num_pram_groups in [1, 2]:
            ...
            sche_cfg = dict(
                ...
                warmup_iters=5,
                warmup_ratio=0.001,
                periods=[10] * 2 if by_epoch else [10] * 6,
...
def test_auto():
    ...
    for num_pram_groups in [1, 2]:
        ...
        sche_cfg = dict(
            type="multistep", steps=[30, 50], warmup_factor=0.001, warmup_iters=10
        )
